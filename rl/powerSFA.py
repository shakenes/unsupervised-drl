##################################################################
# This code is part of a submission to ICLR 2019.
# https://openreview.net/forum?id=rJlHIo09KQ
#
# It implements the PowerSFA method by differentiable whitening
# based on power iterations. Note that this code already has 
# running averages for mean and covariance matrices implemented,
# even though this was not used for generating results in the 
# submission.
##################################################################
...
def compute_whitening_matrix_tf(covariance_matrix, output_dim, n_iterations=50, layer_id=0, random_init=None):
    ...
    R = tf.get_variable("random_vectors" + str(layer_id),
                        initializer=initializer.astype(np.float32),
                        trainable=False)
    whitening_matrix = tf.get_variable("whiteningmatrix" + str(layer_id),
                                       initializer=np.zeros(shape=(output_dim, output_dim)).astype(np.float32),
                                       trainable=False)
    init_custom_op = tf.variables_initializer(var_list=[whitening_matrix, R])
...
class PowerWhitening(Layer):
    ...
    def __init__(self, output_dim=1, n_iterations=100, mixing_coefficient=0.0, distinguish_phases=True, layer_id=0,
                 random_init=None, **kwargs):
        ...
        self.last_mean = tf.get_variable("last_mean" + str(self.layer_id), (output_dim,), tf.float32,
                                         initializer=tf.initializers.zeros, trainable=False)
        self.last_whitening_matrix = tf.get_variable("last_whitening_matrix" + str(self.layer_id),
    ...
    def call(self, input_tensor, layer_id=0, mask=None):
        ...
        self.covariance_matrix = tf.div(tf.matmul(train_input_tensor, train_input_tensor, True, False),
                                        tf.cast(tf.shape(train_input_tensor)[0], tf.float32))
        self.last_covariance_matrix = tf.get_variable("last_covariance_matrix" + str(self.layer_id),
                                                      self.covariance_matrix.shape,
                                                      tf.float32,
                                                      initializer=tf.initializers.identity,
                                                      trainable=False)
        mixed_covariance = self.mixing_coefficient * self.last_covariance_matrix + (
                    1 - self.mixing_coefficient) * self.covariance_matrix
        ...
        with tf.control_dependencies([whitening_matrix]):
            assign_op_mean = tf.assign(self.last_mean, mixed_mean)
            assign_op_mixed = tf.assign(self.last_covariance_matrix, mixed_covariance)
            assign_op_whitening = tf.assign(self.last_whitening_matrix, whitening_matrix)
