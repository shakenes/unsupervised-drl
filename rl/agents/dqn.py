...
class AbstractDQNAgentAE(Agent):

    def __init__(self, nb_actions, memory, gamma=.99, batch_size=32, nb_steps_warmup=1000,
                 train_interval=1, memory_interval=1, target_model_update=10000,
                 delta_range=None, delta_clip=np.inf, custom_model_objects={}, **kwargs):
        super(AbstractDQNAgentAE, self).__init__(**kwargs)
...
class DQNAgentAE(AbstractDQNAgentAE):
    """
    # Arguments 
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        super(DQNAgentAE, self).__init__(*args, **kwargs)
        ...
        self.ae_dir = os.path.join(out_dir, datetime.datetime.now().strftime("CAE_predictions_%Y-%m-%d_%H-%M-%S-%f"))
        os.mkdir(self.ae_dir)

    def get_config(self):
        config = super(DQNAgentAE, self).get_config()
    ...
    def compile(self, optimizer, metrics=[], aemodel=None):
        ...
        loss_out = Lambda(clipped_masked_error, output_shape=(1,), name='loss_dqn')([y_true, y_pred, mask])
        ...
        trainable_model.compile(optimizer=optimizer[0], loss=losses, metrics=combined_metrics)
        print(trainable_model.summary())
        self.trainable_model = trainable_model

        autoencoder_model = Model(inputs=ins, outputs=aemodel.output[1])
        autoencoder_model.compile(optimizer=optimizer[1], loss='mean_squared_error')
        self.autoencoder_model = autoencoder_model
        print(autoencoder_model.summary())

        self.latent_space = Model(inputs=ins, outputs=aemodel.layers[5].output)
    ...
    def forward(self, observation):
        ...
        if self.step % 1000 == 0:
            obs_batch = np.array(observation)
            obs_batch = obs_batch.astype('float32') / 255.
            obs_batch = np.expand_dims(np.expand_dims(obs_batch, 0), 0)
            lat_space = self.latent_space.predict(obs_batch)
            ae_output = self.autoencoder_model.predict(obs_batch)
            try:
                scipy.misc.imsave(os.path.join(self.ae_dir, 'AE_latent_{}_{}.png'.format(self.step, action)),
                                  np.reshape(lat_space[0, :], (64, -1)))
                scipy.misc.imsave(os.path.join(self.ae_dir, 'AE_output_{}_{}.png'.format(self.step, action)),
                                  ae_output[0, 0, :, :])
            except IndexError:
                pass
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            ae_loss = -1.0
            if self.step < 500000:
                self.conv_layers_trainable(model=self.autoencoder_model, trainable=True)
                # add some noise for stability
                ins_noise = ins[0] + np.random.normal(loc=0.0, scale=0.02, size=ins[0].shape)
                ae_loss = self.autoencoder_model.train_on_batch(ins_noise, state0_batch)
    ...
    def conv_layers_trainable(self, model, trainable):
...
# http://arxiv.org/pdf/1312.5602.pdf
# http://arxiv.org/abs/1509.06461
class DQNAgentSFA(AbstractDQNAgent):
    """
    # Arguments
        model__: A Keras model.
        policy__: A Keras-rl policy that are defined in [policy](https://github.com/matthiasplappert/keras-rl/blob/master/rl/policy.py).
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
                 dueling_type='avg', SFAmodel=None, *args, **kwargs):
        super(DQNAgentSFA, self).__init__(*args, **kwargs)
    ...
    def get_config(self):
        config = super(DQNAgentSFA, self).get_config()
        config['enable_double_dqn'] = self.enable_double_dqn
        config['dueling_type'] = self.dueling_type
        config['enable_dueling_network'] = self.enable_dueling_network
        config['model'] = get_object_config(self.model)
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            # generate SFA Batch
            if self.step % (8 * self.train_interval) == 0 and self.step <= 500000:
                SFAexperiences = self.memory.sample(8 * self.batch_size)
                assert len(SFAexperiences) == 8 * self.batch_size

                # Start by extracting the necessary parameters (we use a vectorized implementation).
                SFAstate0_batch = []
                SFAstate1_batch = []
                for e in SFAexperiences:
                    SFAstate0_batch.append(e.state0)
                    SFAstate1_batch.append(e.state1)

                # Prepare and validate parameters.
                SFAstate0_batch = self.process_state_batch(SFAstate0_batch)
                SFAstate1_batch = self.process_state_batch(SFAstate1_batch)

                insSFA = np.empty((2 * SFAstate0_batch.shape[0],
                                   SFAstate0_batch.shape[1],
                                   SFAstate0_batch.shape[2],
                                   SFAstate0_batch.shape[3]))
                for i in range(SFAstate0_batch.shape[0]):
                    insSFA[2 * i, :, :, :] = SFAstate0_batch[i, :, :, :]
                    insSFA[2 * i + 1, :, :, :] = SFAstate1_batch[i, :, :, :]

                self.SFAmodel.train_on_batch(insSFA, sfa_adjacency_matrix(len(insSFA)))

            # Finally, perform a single update on the entire batch. We use a dummy target since
            # the actual loss is computed in a Lambda layer that needs more complex input. However,
            # it is still useful to know the actual target to compute metrics properly.
            ins0 = [state0_batch] if type(self.model.input) is not list else state0_batch

            metrics = self.trainable_model.train_on_batch(ins0 + [targets, masks], [dummy_targets, targets])
            metrics = [metric for idx, metric in enumerate(metrics) if
                       idx not in (1, 2)]  # throw away individual losses
            metrics += self.policy.metrics
            if self.processor is not None:
